...
function editRoomModules(enseigneId, pieceId) {
  ...
  let html = `<div style="margin-bottom:15px; font-style:italic;">Cochez les modules pilotables à distance (IoT).</div>`;
...
function addDefaultModules(enseigneId, pieceId) {
    ...
    piece.modules = [
        { id: "window_1", name: "Fenêtre", type: "window", is_iot: true, state: "closed" },
        { id: "hvac_1", name: "Climatisation", type: "hvac", is_iot: true, state: "off", target_temp: 21 },
        { id: "door_1", name: "Porte", type: "door", is_iot: false, state: "closed" }
    ];
...
function saveRoomModules(enseigneId, pieceId) {
    ...
    .catch(err => {
        console.error(err);
        if (typeof showNotification === 'function') showNotification('Erreur lors de la sauvegarde', true);
    });
}
